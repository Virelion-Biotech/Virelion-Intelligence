...
def source_to_paper(source: SourceRecord, domains: list[str] | None = None) -> PaperRecord:
    return PaperRecord(
        paper_id=f"P-{uuid.uuid4().hex[:12]}", title=source.title, abstract=source.abstract,
        url=source.url, doi=normalize_doi(source.doi), pmid=normalize_pmid(source.pmid),
        pmcid=source.pmcid, publisher=source.publisher, authors=source.authors,
        published_at=source.published_at, domains=domains or [],
...
def run(window_days: int = 7, db_path: str = "data/virelion_intelligence.sqlite3", query_texts: list[str] | None = None) -> RunManifest:
    ...
    try:
        ...
        for query_text in queries:
            q = Query(text=query_text, start=start, end=end, limit=50)
            for adapter in adapters:
                try:
                    records = adapter.search(q); counts["raw_records"] += len(records)
                    if isinstance(adapter, GEOAdapter):
                        for dataset in records:
                            db.upsert_dataset(assess_dataset(dataset)); counts["datasets"] += 1
                        continue
        ...
        top = sorted([p for p in seen if p.relevance_score >= 60], key=lambda p: p.relevance_score, reverse=True)[:20]
        source_lookup = {s.source_id: s for s in source_cache}
        for paper in top:
            source = source_lookup.get(f"pubmed:{paper.pmid}") if paper.pmid else None
            if source is None:
                source = next((s for s in source_cache if paper.doi and s.doi == paper.doi), None)
            if source is None:
            ...
            try:
                ...
                if module_names:
                    opportunity = make_opportunity(
                        ...
                        opportunity_type="INVESTIGATION", evidence_ids=[evidence.evidence_id], virelion_modules=module_names,
                        ...
                        virelion_relevance=min(5, 2.5 + 0.5 * len(module_names)), differentiation=3,
                        translational_potential=3 if "ischemic_heart" in paper.domains else 2,
                    )
                    db.upsert_opportunity(opportunity); counts["opportunities"] += 1
            except Exception:
                counts["errors"] += 1

        manifest.status = "COMPLETE" if counts["errors"] < max(1, counts["raw_records"] // 4) else "PARTIAL"
        manifest.finished_at = utc_now(); manifest.counts = counts
        db.upsert_run(manifest); db.commit()
